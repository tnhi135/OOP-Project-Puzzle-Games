// Day la file khai bao lop, ham game mahjong
...
public:
	...
	virtual void play() 
	{
		...
		Texture t1, t2;
		t1.loadFromFile("C:\\Users\\nnhi\\source\\repos\\OOP_Project\\BaiTapLon_Bai_9_Game_1\\tiles_luhan.png");
		t2.loadFromFile("C:\\Users\\nnhi\\source\\repos\\OOP_Project\\BaiTapLon_Bai_9_Game_1\\background1.jpg");
		Sprite s(t1), sBackground(t2);
		int w = 48, h = 66;
		int stepX = w / 2 - 2, stepY = h / 2 - 2;
		double offX = 4.6, offY = 7.1; // z offset
		Vector3i v1, v2;
		std::vector<Vector3i> moves;


		////load from file////
		std::fstream myfile("C:\\Users\\nnhi\\source\\repos\\OOP_Project\\BaiTapLon_Bai_9_Game_1\\map.txt");
		for (int y = 0; y < 18; y++)
			for (int x = 0; x < 30; x++)
			{
				char a;  myfile >> a;
				...
				if (e.type == Event::MouseButtonPressed)
					if (e.key.code == Mouse::Left)
						for (int z = 0; z < 10; z++)
						{
							...
							int x = int((pos.x - z * offX) / stepX);
							int y = int((pos.y + z * offY) / stepY);
			...
			for (int z = 0; z < 10; z++)
				for (int x = 30; x >= 0; x--)
					for (int y = 0; y < 18; y++)
					{
						...
						s.setPosition(float(x*stepX + z * offX), float(y*stepY - z * offY));
